...
#ifdef CONFIG_FTRACE
void func_trace(word_t pc,word_t target);
#endif
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, IFDEF(CONFIG_FTRACE,func_trace(s->pc,0));s->dnpc = (src1 + imm) &~ (word_t)1; R(rd) = s->pc + 4);
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", lbu    , I, R(rd) = src1 << (imm & 0x1f));
  ...
  // TYPE_J
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, IFDEF(CONFIG_FTRACE,func_trace(s->pc,s->pc+imm));s->dnpc = s->pc; s->dnpc += imm; R(rd) = s->pc + 4);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((SEXT(src1, 32) * SEXT(src2, 32)) >> 0)); 
