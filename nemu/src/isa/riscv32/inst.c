...
#define immU() do {\
       	*imm = SEXT(BITS(i, 31, 12)>> 12,20) ; Log(ANSI_FG_CYAN "imm: %#x\n" ANSI_NONE, *imm);\
} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> imm);
  ...
  // TYPE_B
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne   , B,s->dnpc = s->pc; if(src1 != src2) s->dnpc+=imm);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq   , B,s->dnpc = s->pc; if(src1 == src2) s->dnpc+=imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge   , B,s->dnpc = s->pc; if(src1 <= src2) s->dnpc+=imm);
