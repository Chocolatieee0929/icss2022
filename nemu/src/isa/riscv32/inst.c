...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                  \
	...
	// INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm); stack_call(s->pc, s->dnpc);
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, IFDEF(CONFIG_FTRACE,func_trace(s,s->pc+imm));s->dnpc = (src1 + imm); R(rd) = s->pc + 4);
	...
	// INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, I, s->dnpc = isa_raise_intr(cpu.R[17], s->snpc));
	INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, N, s->dnpc = cpu.pc );
	INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add, R, R(rd) = src1 + src2);
	INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub, R, R(rd) = src1 - src2);
	INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, R, R(rd) = src1 * src2);
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, R(rd) = (((long long)src1 * (long long)src2) >> 32));
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd) = (int)((SEXT((long long)src1, 32) * SEXT((long long)src2, 32)) >> 31));
